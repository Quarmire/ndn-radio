...
log "connect wlu1 (default ns) to $SSID via wpa_supplicant (realtek rejects iw connect)"
sudo ip link set wlu1 down 2>/dev/null
sudo iw dev wlu1 set type managed 2>/dev/null
...
sudo wpa_supplicant -B -i wlu1 -c /tmp/wpa.conf -D nl80211 2>&1 | tail -2
for i in $(seq 1 20); do
  st=$(sudo iw dev wlu1 link 2>/dev/null | grep -i "Connected to")
  [ -n "$st" ] && break; sleep 1
done
echo "STA link: ${st:-NOT CONNECTED}"
sudo iw dev wlu1 link 2>/dev/null | grep -iE "freq|tx bitrate|signal" | head

log "assign IPs"
$NS ip addr add 192.168.99.1/24 dev $AP 2>/dev/null
sudo ip addr add 192.168.99.2/24 dev wlu1 2>/dev/null
sleep 1
log "ping over air (from AP netns to STA)"
$NS ping -c 3 -W 2 192.168.99.2 2>&1 | tail -4

log "iperf3 downlink (AP $AP transmits to STA wlu1)"
iperf3 -s -D -1 2>/dev/null   # server on STA side (default ns / wlu1)
sleep 1
timeout 20 bash -c "$NS iperf3 -c 192.168.99.2 -t 8 -i 2 2>&1" | tail -14

log "negotiated TX rate / aggregation (AP station dump)"
$NS iw dev $AP station dump 2>/dev/null | grep -iE "tx bitrate|tx packets|tx bytes"

log "cleanup"
sudo pkill hostapd 2>/dev/null
sudo iw dev wlu1 disconnect 2>/dev/null
sudo ip netns del ap 2>/dev/null
echo DONE
