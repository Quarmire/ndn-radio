...
mod tests {
    ...
    fn false_positive_rate_matches_measured_curve() {
        ...
        const R: u32 = 12;
        for &(depth, exp_bits) in &[(2usize, 12u32), (4, 19), (6, 23), (8, 30)] {
            let mut fp = 0u32;
            let mut trials = 0u32;
            for r in 0..R {
    ...
    fn ksweep_host_replication() {
        ...
        fn set_bits(bits: &mut [u8; 12], key: &[u8; 16], prefix: &[u8], k: u32) {
            let mut key2 = *key;
            for (b, d) in key2.iter_mut().zip(KEY2_DOMAIN.iter()) {
                *b ^= *d;
            }
        ...
        fn contains(bits: &[u8; 12], key: &[u8; 16], prefix: &[u8], k: u32) -> bool {
            let mut probe = [0u8; 12];
            set_bits(&mut probe, key, prefix, k);
            (0..12).all(|i| probe[i] & !bits[i] & if i == 0 { !0b11 } else { 0xff } == 0)
        }
