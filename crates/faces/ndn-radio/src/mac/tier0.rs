...
mod tests {
    ...
    fn wire_round_trip_sets_reserved_bits() {
        ...
        let wire = f.to_wire();
        assert_eq!(
            wire[0] & RESERVED_MASK0,
            0b0000_0011,
            "I/G+U/L not local-group"
        );
    ...
    /// that chose k. Run with `TIER0_KSWEEP=1 cargo test -p ndn-phy-wifi ksweep -- --nocapture`.
    ///
...
mod golden_vectors {
    ...
    fn render() -> String {
        ...
        out.push_str(
            "# ndn-face-monitor-wifi tier0::golden_vectors; asserted by every implementation.\n",
        );
        out.push_str(
            "# Regenerate: NDN_TIER0_REGEN=1 cargo test -p ndn-face-monitor-wifi tier0_golden\n",
        );
        out.push_str(&format!(
    ...
    fn the_vectors_encode_the_properties_they_exist_to_pin() {
        ...
        assert!(
            d12.may_match(&mask8),
            "a depth-12 name must still match its depth-8 prefix mask"
        );
        ...
        assert!(
            d8.popcount() <= FILL_CAP,
            "and every legitimate shape stays under the cap"
        );
    }
}
