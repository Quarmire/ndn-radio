...
//! `cargo run --example blur_encoding --release -p ndn-phy-wifi --features libusb-backend`

fn main() {
    use ndn_phy_wifi::{OPEN_GROUP_KEY, tier0::positions};
    ...
    let mask_of = |mode: &str, salt: u32, d: usize, r: usize| -> Option<u128> {
        ...
        if k == 0 {
            return None;
        } // level not encoded ⇒ receiver cannot use the Blur here
        let mut m = 0u128;
    ...
    println!(
        "{:<10} {:>5} {:>28} {:>16}",
        "encoding", "bits", "FALSE NEG @ genuine depth", "FP @ disjoint"
    );
    println!(
        "{:<10} {:>5} {:>7}{:>7}{:>7}{:>7} {:>8}{:>8}",
        "", "", "d=2", "d=4", "d=6", "d=8", "reg d=3", "reg d=6"
    );
