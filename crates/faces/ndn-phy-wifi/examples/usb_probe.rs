...
//! ndn-phy-wifi --features libusb-backend`.
        ...
        Ok(mut radio) => {
            ...
            match radio.chip_info() {
                Ok(c) => println!(
                    ...
                    c.chip_id,
                    if c.chip_id == ndn_phy_wifi::CHIP_ID_8822E {
                        "8822E — RTL8812EU"
                    } else {
                        "UNEXPECTED"
                    },
            ...
            if std::env::args().any(|a| a == "--fw") {
                ...
                let fw = ndn_phy_wifi::LibUsbRtl88xxBackend::firmware_nic();
            ...
            if std::env::args().any(|a| a == "--mac-init") {
                ...
                if let Err(e) = radio
                    .download_firmware(ndn_phy_wifi::LibUsbRtl88xxBackend::firmware_nic())
                {
            ...
            if std::env::args().any(|a| a == "--inject") {
                ...
                let bringup = || -> Result<(), Box<dyn std::error::Error>> {
                    ...
                    radio.download_firmware(
                        ndn_phy_wifi::LibUsbRtl88xxBackend::firmware_nic(),
                    )?;
                    radio.mac_init()?;
                    radio.monitor_cfg()?;
                ...
                if ok {
                    use ndn_phy_wifi::{InjectFrame, TxIntent};
                    ...
                    for i in 0..n {
                        ...
                        match rt.block_on(
                            <ndn_phy_wifi::LibUsbRtl88xxBackend as ndn_phy_wifi::FrameIo>::inject(&radio, frame),
                        ) {
                            Ok(()) => sent += 1,
                            Err(e) => {
            ...
            if std::env::args().any(|a| a == "--mac") {
                ...
                match radio.efuse_dump_physical() {
                    Ok(physical) => {
                        ...
                        match ndn_phy_wifi::LibUsbRtl88xxBackend::efuse_decode_logical(
                            &physical,
                        ) {
                            Ok(logical) => {
                                for base in
