...
//! [`WirelessFace`] multiplexing that C5's Wi-Fi (802.11) + BLE (ext-adv) bearers via the shared mux — with
...
//!  app_consumer ─FIB→ WirelessFace ==(Wi-Fi+BLE)==> WirelessFace ─FIB→ register_producer
...
//! over real bearers → a forwarding engine, all on hardware.
//!
//! ```sh
//! WL_A=/dev/cu.usbmodem101 WL_B=/dev/cu.usbmodem11101 \
//!   cargo run --example wireless_c5_engine --features shared-mux -p ndn-face-ble-adv
//! ```
...
/// A WirelessFace over both bearers of ONE C5 (Wi-Fi FrameIo + BLE over the shared mux).
fn build_wireless(port: &str) -> Result<WirelessFace, Box<dyn std::error::Error>> {
    ...
    let bearers: Vec<Arc<dyn WirelessBearer>> = vec![
        Arc::new(TransportBearer::new(MonitorWifiFace::new(FaceId(1), io), BearerKind::Wifi, 1).with_mtu(2272)),
        Arc::new(
            TransportBearer::new(BleAdvFace::new(FaceId(2), ble_backend).ndnts_framing().with_mtu(200), BearerKind::Ble, 3)
                .with_mtu(200),
        ),
    ];
    Ok(WirelessFace::broadcast(WIRELESS_FACE, bearers))
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Producer engine over C5 A's WirelessFace.
    ...
    // Consumer engine over C5 B's WirelessFace: route the prefix OUT the wireless face + strategy.
    ...
    println!("Two C5s, each: ForwarderEngine + soft-prefix-reach-defer over ONE WirelessFace (Wi-Fi+BLE).");
    ...
    println!("consumer engine: fetching {prefix} over the WirelessFace (engine + strategy, retry on loss)...");
    let mut got = None;
    ...
    match got {
        Some(data) => {
            ...
            println!("consumer fetched {} → {:?}", *data.name, String::from_utf8_lossy(&content));
            assert_eq!(*data.name, prefix);
            assert_eq!(content, want);
            println!("✔ WHOLE STACK ON AIR: engine + soft-prefix-reach strategy + WirelessFace (Wi-Fi+BLE, one C5) — Interest→Data roundtrip.");
        }
        None => println!("✗ no roundtrip in 60 attempts — check both C5s run firmware/esp32c5-ndn on the same channel"),
    }
