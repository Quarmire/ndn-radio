...
//! consumer, and a **relay** in between, each a `ForwarderEngine` over ONE `WirelessFace` (Wi-Fi+BLE via the
...
//! producer's Data first. We measure the relay's outbound count (`WirelessFace::tx_counter`) under the flood
...
fn hook(engine: &ndn_engine::ForwarderEngine, prefix: &Name) {
    let strat = ndn_strategy::registry::create_by_name(strategy().as_bytes()).expect("strategy registered");
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (pp, pc, pr) = (ports("WL_PROD", "/dev/cu.usbmodem101"), ports("WL_CONS", "/dev/cu.usbmodem11101"), ports("WL_RELAY", "/dev/cu.usbmodem11401"));
    let prefix: Name = "/ndn/wl/svc".parse().unwrap();

    // Producer.
    let (engine_p, shutdown_p) = EngineBuilder::new(EngineConfig::default()).face(build_wireless(&pp)?).build().await?;
    ...
    let (engine_r, shutdown_r) = EngineBuilder::new(EngineConfig::default()).face(relay_face).build().await?;
    engine_r.fib().add_nexthop(&prefix, WIRELESS_FACE, 0);
    hook(&engine_r, &prefix);

    // Consumer.
    let (engine_c, shutdown_c) = EngineBuilder::new(EngineConfig::default()).face(build_wireless(&pc)?).build().await?;
    ...
    println!("3 C5s (producer / relay / consumer), each engine + {} over a WirelessFace.", strategy());
